...
while { true } do {
...

	{
		...
		_vehicle addAction ["<t color='#009000'>" + localize "STR_HALO_VEH" + "</t> <img size='1' image='res\ui_redeploy.paa'/>", "scripts\client\spawn\do_halo.sqf","",-943,false,true,"","[_target] call is_menuok && [_this, _target] call is_owner && [_target, 'FOB',GRLIB_fob_range] call F_check_near && _target isKindOf 'LandVehicle' && (locked _target == 0 || locked _target == 1)", _distveh];
